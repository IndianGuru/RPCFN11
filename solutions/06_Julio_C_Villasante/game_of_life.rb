class GameOfLife
  ...
  def initialize(args)
    debug '06' if $DEBUG # added for debug by ashbb
    args = {:rows => 20, :columns => 20, :seed => nil, :model => :box } # added for shoes visualization by ashbb
    w = args[:rows]    || 5
    h = args[:columns] || 5
  ...
  def evolve
    ...
    @board = board
    output = Array.new(@rows){Array.new(@columns){0}} # added for shoes visualization by ashbb
    @rows.times{|r| @columns.times{|c| output[r][c] = (board[r][c] ? 1 : 0)}} # added for shoes visualization by ashbb
    output # added for shoes visualization by ashbb
  end
